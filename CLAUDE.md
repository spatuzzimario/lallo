...
- **Voce:** connettore nativo ElevenLabs (non Higgsfield — il wrapper Higgsfield rifiuta voci esterne al suo catalogo preset), voce "Linda Fiore" (voice_id `3DPhHWXDY263XJ1d2EPN`). Modello `eleven_v3` per le parole modello isolate, `eleven_multilingual_v2` per le righe di personalità/istruzioni. Dettagli/personalità/motivazione della scelta in `docs/VOCE_LALLO.md`. Caching in Supabase Storage. In demo si può usare la TTS di sistema come fallback.
